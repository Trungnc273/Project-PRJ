...
public class VerifyCodeServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String actualCode = (String) request.getSession().getAttribute("verificationCode");

        if (actualCode != null && enteredCode.equals(actualCode)) {
            request.setAttribute("email", email);
            request.getRequestDispatcher("ResetPassword.jsp").forward(request, response);
        } else {
            // Thay vì in ra, có thể đặt thông báo vào request attribute
            request.setAttribute("errorMessage", "Code not correct!");
            request.setAttribute("email", email);
